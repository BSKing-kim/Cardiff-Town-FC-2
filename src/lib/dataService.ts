...
export class DataService {
  ...
  // Team Stats (public.team_stats)
  static async getTeamStats(forceRefresh = false): Promise<any[]> {
    if (!forceRefresh) {
      const cached = this.getCached<any[]>("team_stats");
      if (cached) return cached;
    }
    try {
      const { data, error } = await (supabase.from("team_stats") as any).select("*").order("date", { ascending: false });
      if (!error && Array.isArray(data)) {
        return this.setCached("team_stats", data);
      }
    } catch (e) {
      console.warn("Supabase error getting team_stats:", e);
    }
    return this.setCached("team_stats", []);
  }

  static async saveTeamStats(stats: any[]): Promise<void> {
    this.invalidateCache("team_stats");
    try {
      const { error } = await (supabase.from("team_stats") as any).upsert(stats, { onConflict: "id" });
      if (error) console.error("Error saving team_stats:", error);
    } catch (e) {
      console.error("Supabase exception saving team_stats:", e);
    }
  }
